...
# every other caller is rejected. Probes run from inside the compose network (orchestrator container).
#
...
probe() {  # probe <service> <token|""> -> HTTP status of an authenticated JSON-RPC call
  docker compose exec -T -e PROBE_URL="http://$1:9900/" -e PROBE_TOKEN="$2" orchestrator /opt/hermes/.venv/bin/python - <<'EOF'
...
card() {  # card <service> -> HTTP status of the public Agent Card
  docker compose exec -T -e PROBE_URL="http://$1:9900/.well-known/agent-card.json" orchestrator /opt/hermes/.venv/bin/python -c \
...
expect "orchestrator -> recipe-expert" "$(probe recipe-expert "$A2A_TOKEN_ORCHESTRATOR")" 200
expect "orchestrator -> cost-expert" "$(probe cost-expert "$A2A_TOKEN_ORCHESTRATOR")" 200
expect "orchestrator -> marketing-expert" "$(probe marketing-expert "$A2A_TOKEN_ORCHESTRATOR")" 200
...
expect "orchestrator -> researcher rejected" "$(probe researcher "$A2A_TOKEN_ORCHESTRATOR")" 401 403
