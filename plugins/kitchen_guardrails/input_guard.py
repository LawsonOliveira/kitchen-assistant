"""Input guard on orchestrator (D11): the owner's message plus orchestrator's last message (≤ 500 chars) get a Haiku verdict once per
turn; allow and uncertain let the turn run, block answers with the scope message, and any failure blocks with the
infrastructure message. The gateway note of a spreadsheet saved inside KITCHEN_IMPORT_DIR is removed before
