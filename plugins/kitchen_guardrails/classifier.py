...
(plugins.entries.kitchen_guardrails.llm).
"""
...
def guard_timeout_seconds(environ) -> float:
    """KITCHEN_GUARD_TIMEOUT_SECONDS is required and must stay below the hook limit, or the plugin refuses to load."""
    raw = (environ.get("KITCHEN_GUARD_TIMEOUT_SECONDS") or "").strip()
    try:
        seconds = float(raw)
    except ValueError:
        raise ValueError("KITCHEN_GUARD_TIMEOUT_SECONDS must be set to a number of seconds below 30") from None
    if not 0 < seconds < HOOK_LIMIT_SECONDS:
        raise ValueError(f"KITCHEN_GUARD_TIMEOUT_SECONDS={raw} must be above 0 and below {HOOK_LIMIT_SECONDS}")
    return seconds
...
def classify(llm, prompt_file: str, content: str, timeout_s: float) -> Verdict:
    ...
    try:
        result = llm.complete_structured(
            ...
            purpose=f"kitchen_guardrails:{prompt_file}")
