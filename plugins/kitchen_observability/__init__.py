...
# Hermes imports this plugin as hermes_plugins.kitchen_observability (hermes_cli/plugins_loader.py); kitchen_a2a and
# kitchen_guardrails import it by its plain name, which must reach these same modules or the trace state splits in two.
for _suffix, _module in (("", sys.modules[__name__]), (".emit", emit), (".trace", trace)):
    sys.modules["kitchen_observability" + _suffix] = _module
...
def _role() -> str:
    return os.environ.get("KITCHEN_AGENT_ROLE", "unknown")
    ...
    def safe(**kwargs):
        try:
            ...
            if hook.__name__ not in _broken_hooks:
                ...
                log.exception("kitchen_observability hook %s failed; the turn continues without it", hook.__name__)
            return None
