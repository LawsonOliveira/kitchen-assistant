"""Bridge to sabor_guardrails' per-turn cost accounting (D37): what is left of the owner turn and what this agent spent.
...
def _session_costs():
    module = next((module for name, module in list(sys.modules.items()) if name.endswith("sabor_guardrails.cost_cap")), None)
...
def remaining_usd(session_id: str) -> float:
    costs = _session_costs()
    if costs is None:
        return float(os.environ.get("SABOR_TURN_COST_CAP_USD", "5.00"))
