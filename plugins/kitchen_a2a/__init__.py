...
The call graph is fixed: orchestrator calls the three experts; the experts call researcher. Each container declares its
role in KITCHEN_AGENT_ROLE and presents its own caller token (KITCHEN_A2A_TOKEN). Every request and reply is checked
...
# 120 s + repair 75 s < researcher A2A server 270 s < experts→researcher 300 s < expert A2A server 450 s < orchestrator→experts 480 s
PEERS = {
...
ROLE_TOOLS = {
    "orchestrator": ["ask_recipe_expert", "ask_cost_expert", "ask_marketing_expert"],
...
def _trace(session_id: str = "", tool_name: str = "") -> dict:
    """This turn's trace under the calling tool's span (kitchen_observability, D32) and what is left of its cost cap (D37)."""
    from .costs import remaining_usd

    try:
        from kitchen_observability import trace
    ...
    return {**context, "turn_cost_remaining_usd": remaining_usd(session_id)}
...
def _call(peer: str, request: dict, request_schema, response_schema) -> str:
    ...
    token = os.environ.get("KITCHEN_A2A_TOKEN", "")
    if not token:
        return _error("missing_token", "KITCHEN_A2A_TOKEN is not set in this container")
    try:
...
def register(ctx) -> None:
    ...
    if role not in ROLE_TOOLS:
        log.error("kitchen_a2a: KITCHEN_AGENT_ROLE=%r is not one of %s; no A2A tools registered", role, sorted(ROLE_TOOLS))
        return
    schemas = _schemas()
