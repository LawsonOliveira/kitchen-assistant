...
Other plugins use `from sabor_observability.emit import emit` — e.g. emit("guard_input", "input_guard",
...
# USD per million tokens (input, output), Anthropic first-party rates as of 2026-09. The sabor_guardrails cost cap
# uses the same numbers (plugins/sabor_guardrails/NOTES.md); cache reads and writes are counted as input tokens.
...
def _event(span: dict, fields: dict, measured_ms: int) -> dict:
    ...
    return {
        ...
        "agent": fields.get("agent") or os.environ.get("SABOR_AGENT_ROLE") or "unknown", "kind": span["kind"],
...
def _trace_attributes(owner_session_id: str | None):
    """Only fifi's spans carry the owner's session: spans of one trace with different session ids would overwrite it."""
    ...
    try:
        ...
        with _trace_attributes(owner_session_id):  # full content capture for the demo (D32); production would sanitize
            return client.start_observation(trace_context=context, name=_text(name)[:200] or "unknown",
                                            ...
                                            metadata={"agent": os.environ.get("SABOR_AGENT_ROLE", "unknown"), "kind": kind})
    except Exception as error:
        _failed("langfuse", error)
        return None
...
def _start_worker() -> None:
    ...
    with _lock:
        if _worker is None or not _worker.is_alive():
            _worker = threading.Thread(target=_send_forever, name="sabor-cockpit-sender", daemon=True)
            _worker.start()
...
def _post(event: dict) -> None:
    request = urllib.request.Request(
        os.environ.get("SABOR_COCKPIT_URL", "").rstrip("/") + "/events", data=json.dumps(event, default=str).encode(),
        method="POST", headers={"Content-Type": "application/json",
                                "Authorization": f"Bearer {os.environ.get('SABOR_COCKPIT_TOKEN', '')}"})
