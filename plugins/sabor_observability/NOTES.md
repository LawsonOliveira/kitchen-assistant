# sabor_observability — notes (PLAN.md Loop 5)

Verified on 2026-09-13 by reading the Hermes source extracted from the local `sabor-agent:local` image
...
- **Plugin module name.** Hermes imports a directory plugin as `hermes_plugins.<slug>`
  (`hermes_cli/plugins_loader.py: _directory_module_name`), not as `sabor_observability`. A plain
  `import sabor_observability` from another plugin would fail (plugins are not on `sys.path`) or, if it resolved,
  load a second copy with its own trace state. `__init__.py` therefore registers the loaded modules under their plain
  names in `sys.modules` (`sabor_observability`, `.emit`, `.trace`); `test_trace.py` loads the plugin the way Hermes
  does and checks both names reach the same module. Consequence for `sabor_guardrails`: its
  `from sabor_observability.emit import emit` works once this plugin has loaded — list `sabor_observability` before
  `sabor_guardrails` in `plugins.enabled`, or import inside the hook.
...
- **Session-keyed trace state instead of a `ContextVar`.** Every hook and tool handler receives `session_id`, while the
  ...
  `researcher_hooks.py`).
- **Span tree.** fifi has no root span: its model and tool spans are roots of the trace, and fifi's spans carry the
  owner's session id and the trace name through `propagate_attributes` (only fifi's, so spans of one trace never
...
- `from sabor_observability.emit import emit` — e.g. `emit("guard_input", "input_guard", session_id=…,
...
- `from sabor_observability import trace`: `trace.current(session_id)` → `{"trace_id", "turn_cost_remaining_usd"
  (received in the A2A request, None on fifi), …}` for the cost cap. `sabor_a2a._trace()` still sends
  `turn_cost_remaining_usd` from `SABOR_TURN_COST_CAP_USD` (Loop 4 step 6c replaces it).
- The price table lives in `emit.PRICES_USD_PER_MILLION` (USD per million tokens, input/output: `claude-sonnet-5`
  2.00/10.00, `claude-haiku-4-5-20251001` 1.00/5.00, cache tokens counted as input); `sabor_guardrails` has the same
  numbers.
