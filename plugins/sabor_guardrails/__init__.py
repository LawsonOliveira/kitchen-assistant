...
fifi only: input guard (llm_execution middleware), output verifier (transform_llm_output), memory write guard,
...
def _emit(kind: str, name: str, **fields) -> None:
    """Best-effort telemetry through sabor_observability; the guard never depends on it."""
    try:
        import sys

        emit = next((module.emit for key, module in list(sys.modules.items())
                     if key.endswith("sabor_observability.emit") and hasattr(module, "emit")), None)
        if emit:
...
def _owner_and_last_assistant(request: dict) -> tuple[str, str]:
    """The owner's newest message and fifi's message before it, from the provider request."""
...
def _synthetic(text: str, model: str):
    ...
    return Message(id="msg_sabor_guardrails", type="message", role="assistant", model=model or "sabor-guardrails",
...
def register(ctx) -> None:
    ...
    def post_api_request(session_id="", model="", response=None, **_):
        ...
        try:
            ...
            log.error("sabor_guardrails: no price for %r; counting the whole cap so the turn stops", model)
            usd = costs.cap.cap
    ...
    def pre_tool_call(tool_name="", args=None, session_id="", **_):
        try:
            ...
            if decision is None and role == "fifi":
    ...
    def transform_tool_result(tool_name="", args=None, result=None, session_id="", **_):
        try:
            ...
            log.exception("sabor_guardrails: could not record %s result", tool_name)
        return None
