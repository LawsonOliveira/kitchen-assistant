...
- **"Ver o modo de preparo" is an answer, not a refusal.** The accept question offers it; when she picks it, ask ask_recipe_expert "research" for the dish's own `source_url` and show her the steps in her words, then ask the accept question again. Never write the method from memory: the recipe contract keeps the page, not the steps.
